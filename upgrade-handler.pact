...
(module upgrades GOVERNANCE
    ...
    (defcap LOCK:bool (owner:string)
        ;  @managed
        ;  (compose-capability (UPGRADE owner))
        (compose-capability (LOCKING))
    )

    (defcap UNLOCK:bool (owner:string)
        @managed
    ...
    (defun write-blueprint:bool (blueprint-hash:string owner:string)
        (with-capability (LOCK owner) 
            (set-token-lock (read-msg "target-token-id") true)
            (let* (
                    (target-token-balance:decimal (get-balance (read-msg "target-token-id") owner))
                    (attachment-token-balance:decimal (get-balance (read-msg "attachment-token-id") owner))
                ...
                (enforce (= target-token-balance 1.0) "Target token does not belong to received owner")
                (enforce (= attachment-token-balance 1.0) "Attachment token does not belong to received owner")
        ...
        (with-capability (UNLOCK blueprint-hash owner)
            (with-read blueprints blueprint-hash
                ...
                (let* (
                    ...
                    (enforce (= (get-balance target-token-id owner) 1.0) "Target token not owned by specified owner")
                    (enforce (= (get-balance attachment-token-id owner) 1.0) "Attachment token not owned by specified owner")
    ...
    (defun complete-upgrade:bool (blueprint-hash:string owner:string)
        (with-capability (UNLOCK owner)
            (update-blueprint-status blueprint-hash "completed")
            true
        )
    )
    ...
    (defpact perform-upgrade:object{blueprint} (blueprint-hash:string owner:string)
        (step-with-rollback
            (write-blueprint blueprint-hash owner)
