...
(module isoko-royalty-policy-v1 GOVERNANCE
  ...
  (defun write-token-lock:object{token-lock} (token-id:string lock:bool)
    @doc "updates token lock value"
    (with-capability (GOVERNANCE)
      (write token-locker token-id
        {
          "locked": lock,
          "token-id": token-id
        }
  ...
  (defun set-token-lock:object{token-lock} (token-id:string lock:bool)
    @doc "updates token lock value"
      (require-capability (n_f1c962776331c4773136dc1587a8355c9957eae1.upgrades.LOCKING))
