...
(module isoko-collection-policy-v1 GOVERNANCE
  ...
  (defconst COLLECTION-POLICY:string (format "{}" [isoko-collection-policy-v1]))
...
(defun enforce-init:bool (token:object{token-info})
  ...
  (require-capability (INIT-CALL (at "id" token) (at "precision" token) (at "uri" token) COLLECTION-POLICY))
...
(defun enforce-mint:bool
  ...
  (enforce-orchestrator)
  (require-capability (MINT-CALL (at "id" token) account amount COLLECTION-POLICY))
  (let*
      (
